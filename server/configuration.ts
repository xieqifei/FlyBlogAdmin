...
const optional = ['GITHUB_BRANCH', 'POSTS_PATH', 'POST_EXTENSIONS', 'SESSION_AGE', 'COOKIE_SECURE', 'LLM_API_KEY', 'LLM_MODEL', 'LLM_BASE_URL', 'LLM_API_STYLE', 'LANGUAGE', 'R2_ACCOUNT_ID', 'R2_ACCESS_KEY_ID', 'R2_SECRET_ACCESS_KEY', 'R2_BUCKET', 'R2_ENDPOINT', 'R2_PUBLIC_URL'] as const;
...
export function r2Status(environment: NodeJS.ProcessEnv = process.env): R2Status {
  const configured = Boolean(environment.R2_ACCOUNT_ID?.trim() && environment.R2_ACCESS_KEY_ID?.trim() && environment.R2_SECRET_ACCESS_KEY?.trim());
  return { configured, publicUrl: environment.R2_PUBLIC_URL?.trim() || null, defaultBucket: environment.R2_BUCKET?.trim() || null };
}
