...
export default function ImageHosting({ configuration }: { configuration: Configuration }) {
  ...
  if (!configured) {
    return <div className="image-hosting-page">
      ...
      <Card title={t('menu.images')}><Typography.Paragraph type="secondary">{t('sg.var.R2_ACCOUNT_ID')}</Typography.Paragraph><Typography.Paragraph type="secondary">{t('sg.var.R2_ACCESS_KEY_ID')}</Typography.Paragraph><Typography.Paragraph type="secondary">{t('sg.var.R2_SECRET_ACCESS_KEY')}</Typography.Paragraph></Card>
    </div>;
  }
