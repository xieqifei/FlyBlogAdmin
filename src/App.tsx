...
import { ApartmentOutlined, EditOutlined, FileAddOutlined, FileTextOutlined, FolderOutlined, HomeOutlined, LogoutOutlined, MenuFoldOutlined, MenuUnfoldOutlined, ReloadOutlined, RobotOutlined, SettingOutlined, TagsOutlined } from '@ant-design/icons';
...
function Dashboard({ configuration, onLogout }: { configuration: Configuration; onLogout: () => void }) {
  ...
  const load = useCallback(async () => { setLoading(true); setLoadError(''); try { const data = await api<{ posts?: Row[] }>('/api/posts'); if (!Array.isArray(data.posts)) throw new Error('API 服务尚未配置，文章数据暂不可用'); setRows(data.posts); } catch (reason) { setRows([]); setLoadError(reason instanceof Error ? reason.message : '文章载入失败'); } finally { setLoading(false); } }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const dashboard = <div className="dashboard-page">
    ...
    <Card title="最近文章" extra={<Button type="link" onClick={() => setSection('posts')}>查看全部</Button>}><Table<Row> size="small" pagination={false} rowKey="path" dataSource={rows.slice(0, 5)} columns={[{ title: '标题', render: (_, row) => row.title || row.name }, { title: '日期', responsive: ['sm'], render: (_, row) => row.updated || row.date || '—' }, { title: '', width: 72, render: (_, row) => <Button type="link" onClick={() => edit(row.path)}>编辑</Button> }]} /></Card>
  </div>;
  ...
  const posts = <Card title="文章" extra={<Space><Button icon={<FileAddOutlined />} type="primary" onClick={create}>新建</Button><Button icon={<ReloadOutlined />} onClick={load}>刷新</Button></Space>}>
    ...
    <Table<Row> rowKey="path" loading={loading} dataSource={rows} scroll={{ x: 680 }} columns={[{ title: '标题', render: (_, row) => row.title || row.name }, { title: '分类 / 标签', responsive: ['md'], render: (_, row) => <Space wrap>{row.categories?.map((value) => <Tag color="blue" key={`c-${value}`}>{value}</Tag>)}{row.tags?.map((value) => <Tag color="green" key={`t-${value}`}>{value}</Tag>)}</Space> }, { title: '路径', dataIndex: 'path', responsive: ['lg'] }, { title: '操作', width: 90, render: (_, row) => <Button type="link" onClick={() => edit(row.path)}>编辑</Button> }]} locale={{ emptyText: <Empty description={loading ? '正在从 GitHub 加载文章' : '暂无文章'} /> }} />
  </Card>;
...
export default function Root() {
  ...
  useEffect(() => { (async () => { try { const current = await api<Configuration>('/api/config'); if (typeof current.configured !== 'boolean' || !Array.isArray(current.missing)) throw new Error('API unavailable'); setConfiguration(current); if (!current.configured) { setAuthenticated(false); return; } await api('/api/auth/session'); setAuthenticated(true); } catch { setConfiguration({ configured: false, status: {}, missing: ['API 服务'] }); setAuthenticated(false); } })(); }, []);
  if (!configuration || authenticated === undefined) return <div className="boot"><Spin size="large" /></div>;
  if (showSetup) return <main className="setup-shell"><div className="setup-heading"><Typography.Title level={2}>FlyBlog Admin 设置</Typography.Title><Button onClick={() => setShowSetup(false)}>返回登录</Button></div><SettingsGuide configuration={configuration} /></main>;
  if (configuration.configured && !authenticated) return <Login onLogin={() => setAuthenticated(true)} onSetup={() => setShowSetup(true)} />;
