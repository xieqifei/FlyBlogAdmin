...
function Dashboard({ configuration, onLogout }: { configuration: Configuration; onLogout: () => void }) {
  ...
  const posts = <Card className="posts-card" title="文章" extra={<Space><Button icon={<FileAddOutlined />} type="primary" onClick={create}>新建</Button><Button icon={<ReloadOutlined />} onClick={load}>刷新</Button></Space>}>
    ...
    <Table<Row> rowKey="path" tableLayout="fixed" loading={loading} dataSource={filteredRows} pagination={{ position: ['bottomCenter'] }} showSorterTooltip={{ target: 'sorter-icon' }} columns={[
      ...
      { title: '分类', key: 'categories', width: '18%', responsive: ['sm'], sorter: (left, right) => (left.categories || []).join('/').localeCompare((right.categories || []).join('/'), 'zh-CN'), render: (_, row) => <Space wrap>{row.categories?.map((value) => <Tag color="blue" key={value}>{value}</Tag>)}</Space> },
      { title: '标签', key: 'tags', width: '20%', responsive: ['md'], sorter: (left, right) => (left.tags || []).join('/').localeCompare((right.tags || []).join('/'), 'zh-CN'), render: (_, row) => <Space wrap>{row.tags?.map((value) => <Tag color="green" key={value}>{value}</Tag>)}</Space> },
      { title: '编辑日期', key: 'updated', width: 120, responsive: ['sm'], defaultSortOrder: 'descend', sorter: (left, right) => time(left.updated || left.date) - time(right.updated || right.date), render: (_, row) => formatDate(row.updated || row.date) },
      { title: '操作', key: 'actions', width: 88, render: (_, row) => <Space size={4}><Button type="text" aria-label={`编辑 ${row.title || row.name}`} title="编辑" icon={<EditOutlined />} onClick={() => edit(row.path)} /><Popconfirm title="确定删除这篇文章？" description="Git 历史中仍可恢复。" onConfirm={() => removeRow(row)}><Button type="text" danger aria-label={`删除 ${row.title || row.name}`} title="删除" icon={<DeleteOutlined />} /></Popconfirm></Space> },
  ...
  const editorPage = editor ? <div className="article-editor-page">
    ...
    <Card className="editor-metadata-card">
      <div className="editor-form">
        ...
        <Collapse ghost items={[{ key: 'metadata', label: '分类与标签', children: <Grid gutter={[16, 12]}>
          <Col xs={24} md={12}><label>分类<Select mode="tags" tokenSeparators={[',']} value={list(metadata.categories)} onChange={(value) => updateMetadata({ categories: value })} placeholder="输入后回车添加" /></label></Col>
          <Col xs={24} md={12}><label>标签<Select mode="tags" tokenSeparators={[',']} value={list(metadata.tags)} onChange={(value) => updateMetadata({ tags: value })} placeholder="输入后回车添加" /></label></Col>
        </Grid> }]}/>
      </div>
    </Card>
